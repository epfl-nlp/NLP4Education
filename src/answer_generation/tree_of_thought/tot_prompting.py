...
class Generator:
    ...
    def chat(self, prompt):
        ...
        course_name = prompt["problem"]["course_name"]
        system_instruction = f"You are an expert in {course_name}."
        messages.append({"role": "system", "content": system_instruction})

        # Create the prompt
        prompt_content = get_prompt(prompt["problem"]).strip()
...
def parse_args():
    parser = argparse.ArgumentParser(description="Performing tree of thought for the input dataset.")
    ...
    parser.add_argument("--prompt_type", type=str, default="tree_of_thought")
...
def main():
    ...
    logger.info(f"{len(output_responses)} questiones are processed in {time.time() - start :.2f} seconds.")
